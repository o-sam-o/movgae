  ...
  def ProgrammaticLogin(self, captcha_token=None, captcha_response=None, 
    ...
    Args:
      ...
      captcha_response: string (optional) The user's answer to the captch 
                        challenge.
      auth_url: string (optional) A separate URL to authenticate at (used only
                for YouTube services)
      developer_key: string (optional) A string to identify YouTube developers.
                     Required only for YouTube API uploads.
      client_id: string (optional) A string to identify a YouTube API client.
                        Required only for YouTube API uploads.
